    ...
    for attempt in range(max_retries):
        try:
            ...
            for res in sorted_results[:num_results]:
                # 기사 자체에 이미지 URL이 포함되어 있는지 확인
                img_url = res.get('image') or res.get('img_url') or ""
            ...
            # 이미지 처리 (Tavily top-level images 리스트 보완)
            found_images = response.get('images', [])
            for i, article in enumerate(articles):
                if not article.image and i < len(found_images):
                    article.image = found_images[i]
