...
from components.result_section import (
    ...
    render_location_map, render_situation_picture
)
...
def main():
    ...
    if st.session_state.last_result:
        ...
        with col_sub:
            # 여러 지점 표시를 위해 locations 리스트 전달
            locations = getattr(res, 'locations', [])
            if not locations and res.latitude != 0.0:
                # 하위 호환성: locations가 없고 단일 좌표가 있는 경우 리스트로 변환
                locations = [{"name": res.location_name, "lat": res.latitude, "lon": res.longitude}]
            
            render_location_map(locations)
            st.divider()
            render_situation_picture(res.situation_image_url, res.keyword, res.location_name)
    else:
        # 초기 화면 안내
        st.write("---")
        if not search_keys:
