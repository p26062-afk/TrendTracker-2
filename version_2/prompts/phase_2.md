...
- location_name: str (중심 위치 - 행위자보다 사건 발생/영향 지역 우선)
- latitude: float (위도)
- longitude: float (경도)
- situation_image_url: str (전체 상황을 대표하는 사진(Picture) URL)

SearchResult에는 CSV 저장을 위해 Long format(기사 1건=1행)으로 변환하는 `to_dataframe()` 메서드를 추가해주세요.

#### [비즈니스 로직 정의: Location 및 Map 설정]
- **Target-Centric Mapping**: 글로벌 상황 분석 시, 지도의 중심은 '행위자(Actor)'의 위치가 아닌 **'사건의 발생지' 또는 '직접적인 영향이 미치는 대상 지역(Target Area)'**으로 설정해야 함.
    - 예: 미군이 중동에서 작전을 수행하는 경우, 미국 본토가 아닌 '중동 작전 지역'을 Location으로 식별.
- **Situation Preview**: 각 검색 결과는 해당 상황을 시각적으로 설명할 수 있는 대표 사진(`situation_image_url`)을 포함해야 함.
