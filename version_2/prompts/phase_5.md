## Phase 5: 비즈니스 로직 - 서비스 오케스트레이션 (7단계 중 5단계)

---

Phase 5: 각 기능을 조합하여 전체 검색 프로세스를 실행하는 메인 서비스 로직을 구현합니다.

### 1. 뉴스 검색 오케스트레이터 (services/search_orchestrator.py)

#### `execute_news_search(keyword: str, num_results: int = 5) -> SearchResult`

이 함수는 다음 단계별로 동작해야 합니다:

1. **입력값 전처리**: `utils.input_handler.preprocess_keyword()`를 호출하여 키워드를 정제합니다. 
   - 결과가 `None`이면 `AppError("empty_input")`를 발생시킵니다.
2. **검색 키 생성**: `utils.key_generator.generate_search_key()`를 호출하여 고유 ID를 생성합니다.
3. **뉴스 검색**: `services.search_service.search_news()`를 호출하여 Tavily API로부터 기사를 가져옵니다.
   - 결과가 없으면 빈 리스트가 반환되며, 이후 로직을 처리합니다.
4. **AI 요약**: 기사가 있는 경우 `services.ai_service.summarize_news()`를 호출하여 요약문을 생성합니다.
   - 기사가 없으면 "검색 결과가 없습니다."라는 메시지를 요약문으로 사용합니다.
5. **결과 객체 생성**: `SearchResult` 엔티티를 생성합니다.
   - `search_key`: 생성된 키
   - `search_time`: 현재 시간 (`datetime.now()`)
   - `keyword`: 정제된 키워드
   - `articles`: 검색된 기사 리스트
   - `ai_summary`: 생성된 요약문
6. **데이터 저장**: `repositories.search_repository.save_result()`를 호출하여 결과를 CSV에 영구 저장합니다.
7. **결과 반환**: 생성된 `SearchResult` 객체를 반환합니다.

---

### 2. 구현 요구사항
- 앞서 구현한 모든 레이어(domain, services, repositories, utils)를 통합합니다.
- `AppError`가 발생할 경우 상위(UI 레이어)로 전파되도록 둡니다.
- 타입 힌트를 명확히 작성합니다.
...
```bash
# 오케스트레이터 import 테스트
python -c "from services.search_orchestrator import execute_news_search; print('search_orchestrator import 성공')"

# 통합 테스트 (API 키 설정 필요)
# python -c "from services.search_orchestrator import execute_news_search; res = execute_news_search('인공지능'); print(res.ai_summary)"
```
...
- [ ] `from services.search_orchestrator import execute_news_search` import 성공
- [ ] 키워드 입력부터 검색, 요약, 저장까지의 흐름이 에러 없이 연결됨
- [ ] 검색어 미입력 시 `AppError("empty_input")` 발생 확인
- [ ] 결과 객체 내에 `articles`와 `ai_summary`가 올바르게 담겨 있음

---

### ⚠️ 필수 요구사항: 한글 사용

**모든 에러 메시지와 안내 텍스트는 한글로 작성해주세요.**
